...
export async function getDb() {
  ...
  try { await db.exec(`ALTER TABLE reclamations ADD COLUMN type_garantie TEXT;`); } catch(e){}

  try {
    await db.exec(`ALTER TABLE reclamations ADD COLUMN numero_or TEXT;`);
    await db.exec(`ALTER TABLE reclamations ADD COLUMN montant_devis REAL DEFAULT 0;`);
    await db.exec(`ALTER TABLE reclamations ADD COLUMN pieces_list TEXT;`);
    await db.exec(`ALTER TABLE reclamations ADD COLUMN temps_mo INTEGER;`);
    await db.exec(`ALTER TABLE reclamations ADD COLUMN statut_paiement TEXT DEFAULT 'non_paye';`);
    await db.exec(`ALTER TABLE reclamations ADD COLUMN heure_reception TEXT;`);
  } catch (e) {
